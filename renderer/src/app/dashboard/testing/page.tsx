...
export default function TestingPage() {
  ...
  // Fetch test forecast data - will run even with empty userId
  ...
  const handleGenerateForecast = () => {
    ...
    generateForecast({ userId })
      .then(result => {
        console.log("[TestingPage] Generation result:", result);
        if (result && !result.success) {
          setForecastError(result.error || "Failed to generate forecast");
        }
      })
      .catch(error => {
        console.error("[TestingPage] Error during forecast generation:", error);
  ...
  if (userLoading || !userId) {
    return <LoadingState message="Loading test user…" />;
  }

  // Wait for test forecast data
  if (forecastData === undefined) {
    return <LoadingState message="Loading test forecast data..." />;
  }
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  ...
                  <CardDescription className="text-zinc-500 dark:text-zinc-400">Testing environment for forecast predictions</CardDescription>
                </div>
